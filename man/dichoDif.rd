...
\usage{
dichoDif(Data, group, focal.name, method, props=NULL,
...
Either the output of one of the DIF detection methods, or a list of class "dichoDif" with the following arguments:
  ...
  \item{thrSTD}{the value of \code{thrSTD} option.}
...
\details{
 \code{dichoDif} is a generic function which calls one or several DIF detection methods and summarize their output. The possible methods are: \code{"TID"} for
 Transformed Item Difficulties (TID) method (Angoff and Ford, 1973), \code{"MH"} for mantel-Haenszel (Holland and Thayer, 1988), \code{"Std"} for standardization
 (Dorans and Kulick, 1986), \code{"Logistic"} for logistic regression (Swaminathan and Rogers, 1990), \code{"BD"} for Breslow-Day method (Penfield, 2003), 
 \code{"Lord"} for Lord's chi-square test (Lord, 1980), \code{"Raju"} for Raju's area method (Raju, 1990), and \code{"LRT"} for likelihood-ratio test method
 (Thissen, Steinberg and Wainer, 1988).

 If \code{method} has a single component, the output of \code{dichoDif} is exactly the one provided by the method itself. Otherwise, the main  output is a matrix
 ...
 For Breslow-Day method, two test statistics are available: the usual Breslow-Day statistic for testing homogeneous association (Aguerri, Galibert, Attorresi
 and Maranon, 2009) and the trend test statistic for assessing some monotonic trend in the odss ratios (Penfield, 2003). The DIF statistic is supplied by the
 ...
 The output of the \code{dichoDif} function can be stored in a text file by fixing \code{save.output} and \code{output} appropriately. See the help file of
 \code{\link{selectDif}} function (or any other DIF method) for further information.
}
...
\dontrun{
 ...
 dichoDif(verbal, group=25, focal.name=1, method=c("TID","MH","Std",
   "Logistic"), correct=FALSE, thrSTD=0.08, thrTID=1, purify=TRUE,
   save.output = TRUE, output = c("dicho","default"))
