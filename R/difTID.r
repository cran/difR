...
difTID<-function (Data, group, focal.name, props = NULL, thrTID = 1.5, purify = FALSE, nrIter = 10, save.output=FALSE, output=c("out","default")) 
{
...
Q <- thrTID
    ...
    if (purify == FALSE) {
        ...
        RES <- list(Dj = STATS, prop=PROPS,delta=PROV$delta,axisPar=PROV$pars, thr = Q, DIFitems = DIFitems, 
            purification = purify, names = itNames, save.output=save.output,output=output)
    }
    else {
        nrPur <- 0
        difPur <- NULL
        ...
        RES <- list(Dj = stats1, prop=PROPS,delta=prov1$delta,axisPar=prov1$pars, thr = Q, DIFitems = DIFitems, 
            ...
            convergence = noLoop, names =itNames,save.output=save.output,output=output)
    }
    class(RES) <- "TID"
    return(RES)
}
...
if (type==1){
yl<-c(min(c(res$Dj,-abs(res$thr)))-0.1,max(c(res$Dj,abs(res$thr)))+0.1)
if (number==FALSE){
plot(res$Dj,xlab = "Item", ylab = "Perpendicular distance",ylim = yl, pch = pch, main = "Transformed Item Difficulties")
if (is.character(res$DIFitems) == FALSE) points(res$DIFitems, res$Dj[res$DIFitems], pch = pch, col = col)
}
    else {
  plot(res$Dj, xlab = "Item", ylab = "Perpendicular distance", ylim = yl, 
        ...
        if (is.character(res$DIFitems) == FALSE) text(res$DIFitems, res$Dj[res$DIFitems], res$DIFitems,  col = col)
}
abline(h = -abs(res$thr))
abline(h = abs(res$thr))
}
else{
xl<-yl<-c(max(c(min(res$delta),1))-1,min(c(max(res$delta),25))+1)
if (number==FALSE){
plot(res$delta[,1],res$delta[,2],xlab = "Reference group", ylab = "Focal group",xlim=xl,ylim = yl, pch = pch, main = "Delta plot")
if (is.character(res$DIFitems) == FALSE) points(res$delta[res$DIFitems,1],res$delta[res$DIFitems,2], pch = pch,col = col)
}
    else {
  plot(res$delta[,1],res$delta[,2],xlab = "Reference group", ylab = "Focal group",xlim=xl,ylim = yl, col = "white", main = "Delta plot")
        ...
        if (is.character(res$DIFitems) == FALSE) text(res$delta[res$DIFitems,1],res$delta[res$DIFitems,2], res$DIFitems, col = col)
}
...
print.TID<-function (x, ...) 
{
 ...
 pval <- round(abs(res$Dj), 4)
    symb <- symnum(pval, c(0, 0.5, 1, 1.5,Inf), symbols = c("", 
        "*", "**", "***"))
    m1 <- cbind(round(res$prop,4),round(res$delta,4),round(res$Dj, 4))
    ...
    if (res$save.output==FALSE) cat("Output was not captured!","\n")
    else {
