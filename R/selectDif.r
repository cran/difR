selectDif<-function(Data,group,focal.name,method,alpha=0.05,correct=TRUE,thr=0.1,model="2PL",c=NULL,irtParam=NULL,same.scale=TRUE,purify=FALSE,nrIter=10){
res<-switch(method,
...
Raju=difRaju(Data=Data,group=group,focal.name=focal.name,model=model,alpha=alpha,purify=purify,nrIter=nrIter),
Lord=difLord(Data=Data,group=group,focal.name=focal.name,model=model,alpha=alpha,purify=purify,nrIter=nrIter))
return(res)
}
